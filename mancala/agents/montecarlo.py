import random
from collections import defaultdict
from typing import Union
from dataclasses import dataclass

import numpy as np

from mancala.agents.base import BaseAgent
from mancala.state.base import BaseState

class MCTSRootNode():
    def __init__(self, state: BaseState, sims: int = 100, parent = None, parent_action = None):
        self.state = state.clone()
        self.max_sims = sims
        self.parent = parent
        self.parent_act = parent_action
        self._untried_actions = self.state.legal_actions(self.state.turn)
        self.children = []
        self._num_sims = 0
        self._results = defaultdict(int)
        self._results[1] = 0
        self._results[-1] = 0

    def select(self):
        child = self
        if not self.state._done:
            if len(self._untried_actions) != 0:
                child = self.expand()
            else:
                child = self.best_child()
        return child

    def expand(self):
        action = self._untried_actions.pop()
        next_state = self.state.clone().proceed_action(action)
        child = MCTSChildNode(next_state, action)
        self.children = np.append(self.children, child)
        return child

    def simulate(self, child):
        sim_state = child.state.clone()
        while not sim_state._done:
            legal_actions = sim_state.legal_actions(sim_state.turn)
            if legal_actions is not None:
                action = legal_actions[np.random.randint(len(legal_actions))]
                sim_state = sim_state.proceed_action(action)
            else:
                sim_state = sim_state.proceed_action(None)
        return sim_state._winner

    def backpropagate(self, child, winner):
        self._num_sims += 1
        child.visits += 1
        if winner == self.state.turn:
            child.wins += 1
        else:
            child.losses += 1

    def best_child(self, constant=0.1):
        action_weights = [(((a.wins - a.losses) / a.visits) + constant * (np.sqrt(np.log10(self._num_sims) / a.visits))) for a in
                          self.children]
        return self.children[np.argmax(action_weights)]

    def choose_action(self):
        for i in range(self.max_sims):
            child = self.select()
            winner = self.simulate(child)
            self.backpropagate(child, winner)

        return self.best_child().action

@dataclass
class MCTSChildNode:
    state: BaseState
    action: int
    visits: int = 0
    wins: int = 0
    losses: int = 0
    draws: int = 0

class MCTSAgent(BaseAgent):
    """Agent with random choice policy"""

    def __init__(self, id: int, depth: int = 2):
        self._seed = 42
        self.deterministic = False
        self._depth = depth
        self.set_id(id)

    def policy(self, state: BaseState) -> Union[int, None]:
        legal_actions = state.legal_actions(state.turn)
        if legal_actions is None:
            return None
        root = MCTSRootNode(state, sims=self._depth*5)
        return root.choose_action()
